...
namespace core
{
    namespace a_star
    {
        ...
        {
            return coordinates.x < 0 || coordinates.y < 0 || coordinates.x >= tile_field->width() ||
                                                  ...
                                                  gameplay::tile_field_p tile_field)
        {
            ...
            while(!open_set.empty())
            {
                ...
                for(const auto& direction : gameplay::direction_array)
                {
                    ...
                    if(detect_collision(tile_field, new_coordinates) || find_node(closed_set, new_coordinates))
                    {
                        continue;
                    }
