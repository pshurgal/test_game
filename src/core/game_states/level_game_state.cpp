...
namespace core
{
    namespace game_states
    {
        const core::event_handler level_game_state::mouse_button_up_handler = [](SDL_Event& e, game_state_p state) {
            ...
            if(state_converted)
            {
                if(e.button.button == SDL_BUTTON_LEFT && e.button.clicks == 1)
                {
                    ...
                    if(tile.x != INT32_MIN)
                    {
                        state_converted->_player_path = a_star::get_path(
                            state_converted->player->field_position(), tile, state_converted->tile_field);
                    }
                }
        ...
        level_game_state::level_game_state()
        {
            ...
            guard =
                gameplay::create_unit(_guard_point_1.x, _guard_point_1.y, "guard_0", "guard_1", "guard_2", "guard_3");
        ...
        void level_game_state::update()
        {
            ...
            if(std::chrono::duration_cast<std::chrono::milliseconds>(current_time - last_boulder_spikes_update_time)
                   .count() >= 1500)
            {
        ...
        void level_game_state::create_random_boulder_spike()
        {
            ...
            case gameplay::direction_e::LEFT_DOWN:
                start_position = math::vec2(rand() % tile_field->width(), 0);
                break;
            case gameplay::direction_e::LEFT_UP:
                start_position = math::vec2(tile_field->width() - 1, rand() % tile_field->height());
                break;
            case gameplay::direction_e::RIGHT_UP:
                start_position = math::vec2(rand() % tile_field->width(), tile_field->height() - 1);
                break;
            case gameplay::direction_e::RIGHT_DOWN:
                start_position = math::vec2(0, rand() % tile_field->width());
                break;
            }

            boulder_spikes.push_back(gameplay::create_unit<gameplay::boulder_spike>(direction,
                                                                                    start_position.x,
                                                                                    start_position.y,
                                                                                    "boulder_spike_0",
                                                                                    "boulder_spike_1",
